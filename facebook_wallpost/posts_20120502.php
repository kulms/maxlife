	...
	if(checkValues($_REQUEST['value']))
	{
		...
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts WHERE courseid=".$cid." order by p_id desc limit 1");	
		}else{
		$result = mysql_query("SELECT *,
	...
	elseif($_REQUEST['show_more_post']) // more posting paging
	{
		...
		if(isset($cid)){
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts WHERE courseid=".$cid." order by p_id desc limit ".$_REQUEST['show_more_post'].", 10");	
		}else{
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts order by p_id desc limit ".$_REQUEST['show_more_post'].", 10");
		}
		if(isset($cid)){
			$check_res = mysql_query("SELECT * FROM facebook_posts WHERE courseid=".$cid." order by p_id desc limit ".$next_records.", 10");
		}else{
	...
	{	
		...
		if(isset($cid)){
			$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts WHERE courseid=".$cid." order by p_id desc limit 10");
		}else{
		$result = mysql_query("SELECT *,
