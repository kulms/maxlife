	...
	mysql_query("SET character_set_result=NULL");
	...
	elseif($_REQUEST['show_more_post']) // more posting paging
	{
		...
		if(isset($_SESSION["cid"])){
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts WHERE courseid=".$_SESSION["cid"]." order by p_id desc limit ".$_REQUEST['show_more_post'].", 10");	
		}else{
		$result = mysql_query("SELECT *,
	...
	{	
		...
		if(isset($_SESSION["cid"])){
			$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS TimeSpent FROM facebook_posts WHERE courseid=".$_SESSION["cid"]." order by p_id desc limit 10");
		}else{
		$result = mysql_query("SELECT *,
